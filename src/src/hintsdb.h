...
/* All hints db backends must provide these functions */

/* open/close
The actual exim_dbopen*, exim_dbclose* functions are below, they wrap these here. */

/* exim_dbopen returns a pointer to an EXIM_DB, NULL if failed */
...
/* exim_dbput returns nothing useful, it assumes replace mode */
int exim_dbput(EXIM_DB * dbp, EXIM_DATUM * key, EXIM_DATUM * data);

/* exim_dbputb returns EXIM_DBPUTB_OK if successful, EXIM_DBPUTB_DUP otherwise. The
the actual values of EXIM_DBPUTB_OK and EXIM_DBPUT_DUP depend on the backend */
...
/* exim_dbcreate_cursor returns a valid pointer to a EXIM_CURSOR or NULL */
EXIM_CURSOR * exim_dbcreate_cursor(EXIM_DB * dbp);

/* exim_dbdelete_cursor frees the cursor */
